...
class DropReader2(DatasetReader):
    ...
    def __init__(self,
                 ...
                 bert_pretrain_model: str = "/data/package/bert-base-uncased",
                 external_number: List[int] = None,
                 relaxed_span_match_for_finding_labels: bool = True) -> None:
        super().__init__(lazy)
        self._tokenizer = tokenizer or  WordTokenizer()
        self.bert_tokenizer = BertTokenizer.from_pretrained(bert_pretrain_model,unk=False).wordpiece_tokenizer.tokenize
        ...
        self.external_number = external_number or range(1,101) 
        self.external_tokens = [Token(str(number)) for number in self.external_number]
...
#        if "[UNK]" in [token.text for token in passage_tokens]:
#            p_t = self._tokenizer.tokenize(passage_text)
#            print (p_t)
#            print ("*"*20)
#            #print (split_tokens_by_hyphen(passage_tokens))
#            print (split_tokens_by_hyphen(p_t))
#            print ("*"*20)
...
#            print ("*"*20)
        ...
        passage_question_tokens = [Token("[CLS]")] + passage_tokens + [Token("[SEP]")] + question_tokens + [Token("[SEP]")] + self.external_tokens
        ...
        if self.instance_format == "force_add":
            ...
            #直接看passage 和 question中的span,　不再分开看
            ...
            #valid_self_signs_for_external_add_sub_expressions: List[List[int]] = []
            ...
            if answer_type in ["number", "date"]:
                ...
                target_numbers = []
                '''
                    number类答案一般是一个数字，len(answer_texts) == 1
                    date类中可能有多个数字，比如[3,4,2019]代表2019年４月３日.len(answer_texts)==3
                '''
                for answer_text in answer_texts:
                ...
                #    valid_signs_for_add_sub_expressions = self.find_valid_self_add_sub_expressions(numbers_in_passage_question,target_numbers,self.external_number)
    ...
    def make_marginal_bert_drop_instance(passage_question_tokens: List[Token],
        ...
        external_token_field = TextField(external_tokens, token_indexers)
        ...
        if answer_info:
            ...
            if answer_info["answer_spans"]:
                answer_label[0] = 1.0
            #if answer_info["signs_for_external_add_sub_expressions"]:
            #    answer_label[3] = 1.0
            #elif answer_info["signs_for_add_sub_expressions"]:
